...
//    	String grammar1 = "grammar Hello;\n"
//    			+ "r  : 'hello' ID ;\n"
//    			+ "ID : [a-z]+ ;\n"
//    			+ "WS : [ \\t\\r\\n]+ -> skip\n";

    	String grammar2 = "grammar Hello;\n"
    			+ "r  : 'hello' ID ;\n"
    			+ "ID : [a-z]+ ;\n";

//    	new LexerGrammar(grammar1);
    	new LexerGrammar(grammar2);
    }
   ...
   @Test
    ...
    public void suggest_withExplicitNoCasePreference_shouldSuggestBoth() {
        givenGrammar("r: AB", "AB: A B", "fragment A: 'A' | 'a'", "fragment B: 'B' | 'b'").withCasePreference(CasePreference.BOTH).whenInput("").thenExpect("ab", "AB", "aB", "Ab");
    }

    @Test
    public void suggest_withUppercasePreference_shouldSuggestJustUpper() {
        givenGrammar("r: AB", "AB: A B", "fragment A: 'A' | 'a'", "fragment B: 'B' | 'b'").withCasePreference(CasePreference.UPPER).whenInput("").thenExpect("AB");
    }

    @Test
    public void suggest_withLowercasePreference_shouldSuggestJustLower() {
        givenGrammar("r: AB", "AB: A B", "fragment A: 'A' | 'a'", "fragment B: 'B' | 'b'").withCasePreference(CasePreference.LOWER).whenInput("").thenExpect("ab");
    }
    ...
    private LexerAndParserFactory loadGrammar(String... grammarlines) {
        ...
        String grammarText = firstLine + StringUtils.collectionToDelimitedString(Arrays.asList(grammarlines), ";\n") + ";\n" + "WS : [ ]+ -> skip;\\n";
        LexerGrammar lg;
        try {
...
//            g.implicitLexer = lg;
            return new LexerAndParserFactory() {
                ...
                public Lexer createLexer(CharStream input) {
                    return g.createLexerInterpreter(input);
