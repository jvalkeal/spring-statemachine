...
public class IntegrationLockingStateMachineHandlerService<S, E>
	...
	public <V> V handleWhileLocked(String machineId, Function<StateMachine<S, E>, V> machineFunction) {
	...
	public <V> Mono<V> handleReactivelyWhileLocked(String machineId,
		...
		Mono<StateMachine<S, E>> machineMono = Mono.fromSupplier(() -> stateMachineFactory.getStateMachine(machineId))
				.flatMap(m -> {
					try {
						StateMachineContext<S, E> stateMachineContext = stateMachinePersist.read(machineId);
						return restoreStateMachineReactively(m, stateMachineContext);
					} catch (Exception e) {
	...
	private Mono<StateMachine<S, E>> restoreStateMachineReactively(StateMachine<S, E> stateMachine,
		...
		return stateMachine.getStateMachineAccessor().withRegion().resetStateMachineReactively(stateMachineContext)
